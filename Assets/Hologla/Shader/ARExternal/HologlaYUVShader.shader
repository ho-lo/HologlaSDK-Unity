...
https://bitbucket.org/Unity-Technologies/unity-arkit-plugin/src/default/LICENSES/MIT_LICENSE
...
Shader "Hologla/ARKitCameraShader"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			//Added by Hologram(Following 2 lines).
			float _widthRatio = 1.0;
			float _widthOffset = 0.0;
			...
			TexCoordInOut vert (Vertex vertex)
			{
				...
				o.texcoord.x = (_DisplayTransform[0].x * texX + _DisplayTransform[1].x * (texY) + _DisplayTransform[2].x) * _widthRatio + _widthOffset;
